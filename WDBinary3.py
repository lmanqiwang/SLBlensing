...
class WDBinary3:
    def __init__(self, mass_wd, r_wd, l_wd, mass_star, r_star, l_star, ecc, a, d, omega=np.pi, inc=90*u.deg, period=0.0*u.day, cycles=1.0, limb_darkening_wd=[0.0, 0.0], limb_darkening_star=[0.0, 0.0], N=1000):
        ...
        # self.t = (np.linspace(0, cycles * self.P.value, N) * u.yr).to(u.day)
        t_offset = -0.25 * self.P.to(u.day)
    ...
    def geometric_flux(self):
        ...
        ld_tra, ld_occ, ld_tra_cont = self.geometric_limb_darkening(u1, u2, u3, u4)  
        A = self.amp
        Z = self.Z
        ...
        # amplification
        lens_mask = Z.value > 0
        flux[lens_mask] += A[lens_mask] * F_star * ld_tra_cont[lens_mask]

        # transit
        flux -= k**2 * alpha_tra * F_star * ld_tra

        # occult
        flux -= alpha_occ * F_lens * ld_occ
    ...
    def geometric_limb_darkening(self, u1, u2, u3, u4):
        ...
        transit_mask = np.nonzero(self.alpha_tra)
        occult_mask = np.nonzero(self.alpha_occ)

        ld_tra = np.ones(self.t.size) 
        ld_occ = np.ones(self.t.size)

        # but actually is just proportional to, need elliptic integrals to be accurate
        I_int_tra = (1 - u1/3 - u2/6) * np.pi # integral of I
        I_int_occ = (1 - u3/3 - u4/6) * np.pi 

        ld_tra[transit_mask]= (1 - u1 * (1-mu[transit_mask]) - u2 * (1-mu[transit_mask])**2) / I_int_tra  # I / integral of I
        ld_tra_cont = (1 - u1 * (1-mu) - u2 * (1-mu)**2) / I_int_tra

        ld_occ[occult_mask] = (1 - u3 * (1-mu[occult_mask]) - u4 * (1-mu[occult_mask])**2) / I_int_occ


        return ld_tra, ld_occ, ld_tra_cont
